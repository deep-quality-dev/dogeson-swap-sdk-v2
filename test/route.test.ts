import { Token, WETH, ChainId, Pair, TokenAmount, Route, ETHER, RouterType } from '../src'

describe('Route', () => {
  const token0 = new Token(ChainId.MAINNET, '0x0000000000000000000000000000000000000001', 18, 't0')
  const token1 = new Token(ChainId.MAINNET, '0x0000000000000000000000000000000000000002', 18, 't1')
  const weth = WETH[ChainId.MAINNET]
  const pair_0_1 = new Pair(RouterType.sphynx, new TokenAmount(token0, '100'), new TokenAmount(token1, '200'))
  const pair_0_weth = new Pair(RouterType.sphynx, new TokenAmount(token0, '100'), new TokenAmount(weth, '100'))
  const pair_1_weth = new Pair(RouterType.sphynx, new TokenAmount(token1, '175'), new TokenAmount(weth, '100'))
